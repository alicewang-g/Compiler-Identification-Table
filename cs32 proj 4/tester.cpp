...
const char* COMMAND_FILE_NAME = "/Users/alicewang/Documents/cs32/cs32 proj 4/cs32 proj 4/commands.txt";

class SlowIdentifierTable
{
  public:
    void enterScope();
    bool exitScope();
...
struct Command
{
    ...
    virtual ~Command() {}
    virtual void execute(IdentifierTable& idt) const = 0;
    virtual bool executeAndCheck(IdentifierTable& idt, SlowIdentifierTable& sidt) const = 0;
    string m_line;
    int m_lineno;
};
...
struct EnterScopeCmd : public Command
{
    EnterScopeCmd(string line, int lineno)
     : Command(line, lineno)
    {}
    virtual void execute(IdentifierTable& idt) const
    {
        idt.enterScope();
    }
    virtual bool executeAndCheck(IdentifierTable& idt, SlowIdentifierTable& sidt) const
    {
        idt.enterScope();
        sidt.enterScope();
        return true;
...
struct ExitScopeCmd : public Command
{
    ...
    virtual bool executeAndCheck(IdentifierTable& idt, SlowIdentifierTable& sidt) const
    {
        return idt.exitScope() == sidt.exitScope();
    }
};
...
struct DeclareCmd : public Command
{
    ...
    virtual void execute(IdentifierTable& idt) const
    {
        idt.declare(m_id, m_lineNum);
    }
    virtual bool executeAndCheck(IdentifierTable& idt, SlowIdentifierTable& sidt) const
    {
        return idt.declare(m_id, m_lineNum) == sidt.declare(m_id, m_lineNum);
    }
    string m_id;
    int m_lineNum;
};
...
struct FindCmd : public Command
{
    ...
    virtual bool executeAndCheck(IdentifierTable& idt, SlowIdentifierTable& sidt) const
    {
        return idt.find(m_id) == sidt.find(m_id);
    }
    string m_id;
};
...
{
    IdentifierTable idt;
    SlowIdentifierTable sidt;
    for (size_t k = 0; k < commands.size(); k++)
    {
          // Check if command agrees with our behavior

        if (!commands[k]->executeAndCheck(idt, sidt))
        {
            ostringstream msg;
...
void SlowIdentifierTable::enterScope()
{
...
bool SlowIdentifierTable::exitScope()
{
...
bool SlowIdentifierTable::declare(const string& id, int lineNum)
{
    if (id.empty())
        return false;
...
int SlowIdentifierTable::find(const string& id) const
{
    if (id.empty())
        return -1;
